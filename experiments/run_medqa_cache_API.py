...
from gcache_data.data import load_medqa
from gcache_data.utils import normalize_answer, extract_gpqa_answer


def load_medqa_data(json_path: str) -> List[dict]:
    """Load MedQA dataset"""
    dataset = list(load_medqa(json_path))
    processed = []
    for item in dataset:
        processed.append({
            'task': item['question'],
...
def parse_args():
    ...
    parser.add_argument('--domain', type=str, default="medqa")
...
async def main():
    ...
    # Load dataset
    print("📥 Loading MedQA from JSON...")
    dataset = load_medqa_data(args.dataset_json)
    ...
    # Setup agents
    if args.use_cache:
        # Option 1: Diverse medical agents (recommended for MedQA)
        agent_names = ['MedicalExpertCacheV2', 'ClinicalAnalystCacheV2', 'CodeAgentCacheV2', 'MedicalResearcherCacheV2']
        decision_method = 'FinalReferCacheV2'
        print(f"✅ Using diverse cache-enabled agents: Medical Expert, Clinical Analyst, Code, Medical Researcher")
        
        # Option 2: Uniform agents (uncomment to use)
        # agent_names = ['MathSolverCacheV2'] * sum(args.agent_nums)
        # decision_method = 'FinalReferCacheV2'
        # print(f"✅ Using cache-enabled agents (all Medical Expert)")
    else:
